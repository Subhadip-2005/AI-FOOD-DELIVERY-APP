...
const Home = () => {
  ...
  const toAnalyze = restaurants.filter(
    (r) => !r.reviewSentiment && r.reviews && r.reviews.length > 0
  );

  toAnalyze.forEach((restaurant, index) => {
    setTimeout(() => {
      dispatch(analyzeReviews(restaurant._id));
    }, index * 4000); // stagger by 4 seconds per restaurant
  });
